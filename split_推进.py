...
def _split_text_with_regex_from_end(
    ...
    if separator:
        if keep_separator:
            ...
            splits = ["".join(pair) for pair in zip(_splits[0::2], _splits[1::2])]
            if len(_splits) % 2 == 1:
                splits += [_splits[-1]]
        else:
...
class ChineseRecursiveTextSplitter(RecursiveCharacterTextSplitter):
    def __init__(self, separators: Optional[List[str]] = None, keep_separator: bool = True,
                 is_separator_regex: bool = True, **kwargs: Any):
    ...
    def _split_text(self, text: str, separators: List[str], page_number: int) -> List[Dict[str, Any]]:
        final_chunks = []
        ...
        for s in splits:
            chunk = self._length_function(s) < self._chunk_size
            if chunk:
                final_chunks.append(s)
            else:
                if final_chunks:
                    merged_text = self._merge_splits(final_chunks, separator if self._keep_separator else "")
                    for mt in merged_text:
                        final_chunks.append({"text": mt, "page": page_number})
                    final_chunks = []
                final_chunks.append(s)

        return [{"text": re.sub(r"\n{2,}", "\n", s.strip()), "page": page_number} for s in final_chunks if s.strip()]
...
if __name__ == "__main__":
    ...
    for inum, text in enumerate(ls):
        ...
        chunks = text_splitter.split_text(text,separators,inum)
        for chunk in chunks:
            print(chunk)